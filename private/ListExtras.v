...
Lemma remove_not_in_neq :
  forall A (x : A) y l eq_dec,
    x <> y ->
    ~ In x (remove eq_dec y l) ->
    ~ In x l.
Proof with eauto.
  introv Hneq HnIn.
  introv contra. apply HnIn.
  apply remove_in_neq...
Qed.
...
Lemma filter_snoc_true :
  forall A (x : A) l p,
    p x = true ->
    filter p (snoc l x) = snoc (filter p l) x.
Proof with eauto.
  introv Hp.
  induction l; crush.
  cases_if...
...
Lemma filter_snoc_false :
  forall A (x : A) l p,
    p x = false ->
    filter p (snoc l x) = filter p l.
Proof with eauto.
  introv Hp.
  induction l; crush.
Qed.
