...
Proof with eauto using remove_not_in_neq.
  ...
  expr_cases(induction e) Case;
    simpls; try solve[rewrite IHe; eauto]...
  + Case "EVar".
...
Lemma freeBIds_subst_in :
  forall l x v e,
    In l (freeBIds (subst x v e)) ->
    In l (freeBIds v) \/ In l (freeBIds e).
Proof with eauto using in_or_app.
  introv HIn.
  expr_cases(induction e) Case; simpls...
  + Case "EVar".
...
Ltac heap_rewrite :=
  match goal with
    ...
    | H : context[length (heapUpdate _ _ _)] |- _ =>
      rewrite heapUpdate_length in H; try omega; eauto
    | _ : _ |- context[length (heapUpdate _ _ _)] =>
      rewrite heapUpdate_length; try omega; eauto
    | H : context[length (heapExtend _ _)] |- _ =>
      rewrite snoc_length in H;
      try rewrite snoc_length; try omega; eauto
    | _ : _ |- context[length (heapExtend _ _)] =>
      rewrite snoc_length; try omega; eauto
...
Tactic Notation "heap_case_core" :=
  unfolds LH;
  unfolds conv;
  (* Repeat heap_rewrite, but don't branch *)
  repeat let n := numgoals in
         repeat heap_rewrite;
         let n' := numgoals in
         guard n' = n;
         repeat inv_eq.
...
Tactic Notation "hauto'" int(n) :=
  ...
  (match goal with
    ...
    | _ : _ |- context[LH (heapUpdate _ ?id1 _) ?id2] =>
      heap_case id1 id2
    | _ : _ |- context[conv (heapUpdate _ ?id1 _) ?id2] =>
      heap_case id1 id2
    ...
    | _ : context[LH (heapUpdate _ ?id1 _) ?id2] |- _ =>
      heap_case id1 id2
    | _ : context[conv (heapUpdate _ ?id1 _) ?id2] |- _ =>
      heap_case id1 id2
    ...
    | _ : _ |- context[LH (heapExtend ?H _) ?id] =>
      heap_case' id (length H)
    | _ : _ |- context[conv (heapExtend ?H _) ?id] =>
      heap_case' id (length H)
    ...
    | H1 : context[LH (heapExtend ?H _) ?id] |- _ =>
      heap_case' id (length H)
    | H1 : context[conv (heapExtend ?H _) ?id] |- _ =>
      heap_case' id (length H)
...
Tactic Notation "hauto" := hauto' 5.

Ltac destruct_actor a :=
  destruct a as [[[[?l ?L] ?C] ?Q] ?e].

Tactic Notation "find_actor" ident(id) :=
  match goal with
    | Hlt : id < length ?H |- _ =>
      let a := fresh "a" in
      eapply heapLookup_lt in Hlt as [a ?Hlookup]; destruct_actor a
    | _ => fail "No such actor"
  end.
